...
from procesamiento_entrada.procesamiento_archivo import FileManager, Dictlexemas
from procesamiento_entrada.lexema import Lexema
from procesamiento_entrada.tipo_token import TokenType


class MinimalTokenizer:
    def __init__(self, oracion: List[str]):
    ...
    def buscar_lexemas(self):
        oracion_to_map = deque(self.oracion)
        self.tokenized_lex = []
        self.no_tokenized_lex = []
        processed_lexemas = {}
        while oracion_to_map:
            palabra = oracion_to_map[0]
            if palabra in self.dictlexemas:
                lexemas = list(self.dictlexemas[palabra].values())
                lexema = self.__buscar_mejor_match(list(oracion_to_map), lexemas)
                if lexema:
                    ...
                    # Remueve los elementos procesados de oracion_to_map
                    for _ in range(lexema.length):
                        oracion_to_map.popleft()
                else:
                    self.no_tokenized_lex.append(oracion_to_map.popleft())
            else:
                self.no_tokenized_lex.append(oracion_to_map.popleft())

    def __buscar_mejor_match(self, oracion_to_map: List[str], lexemas: List[Lexema]) -> Optional[Lexema]:
        oracion = '_'.join(oracion_to_map)
        search_results: List[Lexema] = [
            lexema for lexema in lexemas if self.__esta_en_orden(lexema.id, oracion)
        ]
