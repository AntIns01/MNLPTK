...
class EditLexeme(tk.Toplevel):
    def __init__(self, parent, word):
        ...
        self.result = None
        self.elimino = False
        print("self.word" + str(self.word))
        # Configuración básica del diálogo
        self.title('Editar lexema')
        self.geometry('400x300')

        # Variables para almacenar datos
        self.text = tk.StringVar(value=self.word[1])
        self.selected_token_type = tk.StringVar(value=self.word[2])
        self.weight = tk.IntVar(value=word[3])

        # Widgets del diálogo
        self.label = ttk.Label(self, text='Edite palabra o frase:')
        ...
        self.button_ok = ttk.Button(self, text='Modificar', command=self.accept)
        self.button_eliminar = ttk.Button(self, text='Eliminar', command=self.eliminar)
        ...
        self.button_eliminar.pack(pady=10, side=tk.LEFT, padx=10)
    ...
    def eliminar(self):
        try:
            key = self.word[0]
            FileManager.eliminar_lexemas(key)
            self.result = None
            self.elimino = True
            self.destroy()
